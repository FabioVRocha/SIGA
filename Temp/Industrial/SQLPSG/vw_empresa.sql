...
'::text) AS empresa_observacao,
    ...
    c2.cmerepcod AS empresa_representante,
    v.vennome AS empresa_representante_nome
   FROM empresa e
     ...
     LEFT JOIN cmempre2 c2 ON c2.cmempresa = e.empresa
     LEFT JOIN vendedor v ON v.vendedor = c2.cmerepcod
  GROUP BY e.empresa, e.empnome, e.empfanta, e.empbairro, e.empemail, e.empmainfe, e.empmaibol, e.empmaicte, e.empcidade, e.empdtalt, e.emphralt,
        ...
        CASE
            ...
            WHEN e.empexp = 'S'::bpchar THEN 'Estrangeira'::text
            ELSE NULL::text
        END, e.emplimcre, e.emprua, e.empcep, e.empcomple, e.empendcob, e.empcepcob, e.empcobcomp, e.empbaicob, e.empcidcob, e.emptelef, e.empcelular, e.empvalid, e.empdpess, e.empdemptip, e.empwhats, c2.cmerepcod, v.vennome
  ORDER BY e.empnome;

ALTER TABLE vw_empresa
  OWNER TO postgres;
