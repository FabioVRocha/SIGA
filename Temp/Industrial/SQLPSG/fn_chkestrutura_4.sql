-- Function: fn_chkestrutura_4(character, text, character, integer, integer, text)

-- DROP FUNCTION fn_chkestrutura_4(character, text, character, integer, integer, text);

CREATE OR REPLACE FUNCTION fn_chkestrutura_4(
    poriginal character,
    ppercorridos text,
...
loop
  ...
  Else  
	...
	if precursao = 0 then
        ...
        ppercorridos_ = fn_chkestrutura_3(poriginal, entrada , tmp_record.estfilho, precursao, pnivel + 1, pcaminho || '->' || tmp_record.estfilho);
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION fn_chkestrutura_4(character, text, character, integer, integer, text)
  OWNER TO postgres;
