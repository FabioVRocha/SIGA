...
CREATE OR REPLACE VIEW pw_pedido_venda AS 
 SELECT pp.pedido AS pedidovenda_codigo_pk,
    ...
    round((( SELECT prod.proqtdger
           FROM produto prod
          WHERE prod.produto = pp.pprproduto)) * pp.pprquanti, 4) AS pedidovenda_qtde_gerencial_produto,
    round((( SELECT prod.proliqui
           FROM produto prod
          WHERE prod.produto = pp.pprproduto)) * pp.pprquanti, 4) AS pedidovenda_qtde_peso_liquido_produto,
     ...
     LEFT JOIN ( SELECT pedido.pedido,
           ...
           FROM pedido
             LEFT JOIN ( SELECT DISTINCT ON (ve.coppedido) ve.coppedido,
                    ve.vendedor, ve.coptiprep
                   FROM comiped ve where ve.coptiprep <> 'D'
