-- Function: pw_fn_prazo_medio_estocagem(date, date, text)

-- DROP FUNCTION pw_fn_prazo_medio_estocagem(date, date, text);

CREATE OR REPLACE FUNCTION pw_fn_prazo_medio_estocagem(pdataini date, pdatafim date, pproduto text)
  RETURNS numeric AS
$BODY$
declare
	Cusven    record;
	...
	prazomedioestocagem    numeric(15,4);
...
begin	
	...
	if coalesce(custoprodutosvendidos,0) <> 0 then
		prazomedioestocagem = (estoquemedio / custoprodutosvendidos) * vPeriodo;
	end if;
	--raise notice '**prazomedioestocagem %', prazomedioestocagem;
	return prazomedioestocagem;
end;
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION pw_fn_prazo_medio_estocagem(date, date, text) SET search_path=public, pg_temp;

ALTER FUNCTION pw_fn_prazo_medio_estocagem(date, date, text)
  OWNER TO postgres;
