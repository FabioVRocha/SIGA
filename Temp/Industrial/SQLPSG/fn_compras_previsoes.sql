-- Function: fn_compras_previsoes(integer, character)

-- DROP FUNCTION fn_compras_previsoes(integer, character);

CREATE OR REPLACE FUNCTION fn_compras_previsoes(IN compra_param integer, IN condicao_param character)
		...
		if (pcmdtbcp) <> 'F' then --Caso seja Data base para Calculo das Previsoes de Pagamento seja diferente de Data Fatura, deve realizar o processo pelos itens do pedido, caso contrario, sera pelo cabecalho.
			  ...
			  loop
				...
				if (infosped.comvlfrete <> 0 or infosped.comperfret <> 0) and infosped.comrespons ='F' then
					if (infosped.comperfrete <> 0) then
						valorfinalimpo := round((valorfinal * infosped.comperfret <> 0) / 100,2);
					else
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION fn_compras_previsoes(integer, character)
  OWNER TO postgres;
