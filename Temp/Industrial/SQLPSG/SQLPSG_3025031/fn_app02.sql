...
DECLARE
  ...
  cCentroCusto char(5);
  cPlanoContas char(15);
  cTipoPlanilhamento char(1);
BEGIN 
   nn = 1;
   cjsontxt = cjson;
   ...
   for cjsontxt2 in (select * from json_array_elements(cjsontxt::json -> 'results') ) loop --Faz um laco para ler todas as posicoes
	if nn <= jsonTam then
	   ...
	   cCentroCusto = cjsontxt2    ->> 'CentroCusto';
	   cPlanoContas = cjsontxt2    ->> 'PlanoContas';
	   cTipoPlanilhamento = cjsontxt2 ->> 'TipoPlanilhamento';
	   
	   --Grava o temporario para o usuario dos parametros
	   insert into umapphor(umpousu, umposeq, umpoordpro, umpoproc, umpodepo, umpoqtde, umpotxt, umpoappusu, umpodtini, umpohrini, umpoopera, umpomaq, umpofase, umpodtfim, umpohrfim, umpohrtot, umpomoti, umpomateri, umpolotexe, umponumres, umprodut, umpotpmov, umpodocum, umpoobserv,umpofuncio, umpolotpro, umpoforpas, umpocc, umpopl, umpopartot) values
	       (cusuario, nn, cOrdemProcesso, cProcesso, cDeposito, cQuantidade, '', cUsuarioJson, cDataInicial, cHoraInicial, cOperacao, cMaquina, cFase, cDataFinal, cHoraFinal, cHoraTotal, cMotivo, cMaterial, cLoteExecucao, cNumReserva, cProduto, cTipomovimento, cDocumento, cObservacao,cFuncionario, cLoteProducao, cFormaPassagem, cCentroCusto, cPlanoContas, cTipoPlanilhamento);
	else
	   exit;
	end If;
	nn = nn + 1;
