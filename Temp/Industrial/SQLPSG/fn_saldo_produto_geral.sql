-- Function: fn_saldo_produto_geral(date, character, character, numeric, numeric)

-- DROP FUNCTION fn_saldo_produto_geral(date, character, character, numeric, numeric);

CREATE OR REPLACE FUNCTION fn_saldo_produto_geral(pdata date, pproduto character, pdeposito character, pcontrole numeric, psequencia numeric)
  RETURNS numeric AS
$BODY$
DECLARE
	vtpsaldo char(1);
...
ALTER FUNCTION fn_saldo_produto_geral(date, character, character, numeric, numeric) SET search_path=public, pg_temp;
  
ALTER FUNCTION fn_saldo_produto_geral(date, character, character, numeric, numeric)
  OWNER TO postgres;
